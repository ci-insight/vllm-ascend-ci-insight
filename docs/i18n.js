const I18N = {
  en: {
    title: "vllm-ascend CI Insight",
    searchPlaceholder: "Search PR title, number...",
    allSeverities: "All Severities",
    allCategories: "All Categories",
    severity: {
      critical: "Critical",
      high: "High",
      medium: "Medium",
      low: "Low",
      unknown: "Unknown",
    },
    category: {
      code: "Code Bug",
      build: "Build/CI Script",
      infra: "Infrastructure",
      test: "Test Case",
      lint: "Lint/Format",
      compat: "Compatibility",
      perf: "Performance",
      other: "Other",
    },
    refresh: "Reload Data",
    loading: "Loading...",
    loadFailed: "Failed to load reports.",
    noReports: "No reports found.",
    failed: "failed",
    reports: "reports",
    totalFailedJobs: "Failed Jobs",
    avgConf: "Avg Confidence",
    severityBreakdown: "Severity Breakdown",
    topWorkflows: "Top Failing Workflows",
    categoryBreakdown: "Category Breakdown",
    dailyTrend: "Daily Failure Trend",
    recentReports: "Recent Reports",
    pr: "PR",
    jobs: "Jobs",
    severityLabel: "Severity",
    categoryLabel: "Category",
    date: "Date",
    affectedRuns: "Affected CI Runs",
    noRuns: "No runs recorded.",
    analysis: "Analysis",
    failedJobs: "failed jobs",
    noAnalysis: "No analysis results.",
    errorSnippets: "Error Snippets:",
    rootCause: "Root Cause",
    relatedFiles: "Related Files:",
    fixSuggestions: "Fix Suggestions:",
    noRootCause: "No root cause analysis.",
    detailFailed: "Failed to load",
    author: "Author",
    analyzed: "CI Triggered",
    lang: "中文",
    confidence: "confidence",
    chartLoading: "Loading charts...",
    classified: "classified",
    unclassified: "unclassified",
    clickToDrill: "Click to drill down",
    categoryDetailTitle: "Category Details",
    // CI Execution Analysis
    tabAnalysis: "Problem Analysis",
    tabCIStats: "CI Execution",
    ciAvgDuration: "Avg Duration",
    ciMedianDuration: "P50 Duration",
    ciP90Duration: "P90 Duration",
    ciP20Duration: "P20 Duration",
    ciTotalRuns: "Total Runs",
    ciSuccessRate: "Success Rate",
    ciQueueTime: "Queue Time",
    ciQueueMeasured: "queue samples",
    ciNotCollected: "Not collected",
    ciQueueNotCollectedHint: "Queue time requires GitHub job created_at. Re-exported older job details may not include this field until the run is re-fetched.",
    ciDurationDist: "Job Duration Distribution (P50/P90/P20)",
    ciQueueWait: "Job Queue Wait Time (started - job created)",
    ciSuccessByWF: "Success Rate by Workflow",
    ciSlowestJobs: "Slowest Jobs (Top 10)",
    ciDuration: "Duration",
    ciQueueWaitLabel: "Queue Wait",
    ciSuccess: "success",
    ciFailed: "failed",
    ciSkipped: "skipped",
    ciSeconds: "s",
    ciMinutes: "min",
    ciTotalJobs: "Total Jobs",
    ciJobP50: "Job P50",
    ciJobP90: "Job P90",
    ciJobP20: "Job P20",
    ciWfTotal: "Workflows",
    ciWfAvgWC: "Avg Workflow Duration",
    ciWfP50: "Workflow P50",
    ciWfP90: "Workflow P90",
    ciWfAvgJobs: "Avg Jobs/Workflow",
    ciWfEfficiency: "Parallelism",
    ciWfSuccess: "Workflow Success",
    ciTableTitle: "Workflow Runs Detail",
    tabHealth: "Health",
    healthScoreLabel: "Health Score",
    successRateLabel: "Success Rate",
    pipeline_pr_e2e: "PR CI",
    pipeline_nightly: "Nightly",
    pipeline_build: "Build/Release",
    pipeline_other: "Other",
    allPipelines: "All Pipelines",
    effort_low: "Easy",
    effort_medium: "Medium",
    effort_high: "Hard",
    ciThWorkflow: "Workflow",
    ciThStatus: "Status",
    ciThJobs: "Jobs",
    ciThQueueTime: "Max Queue Wait",
    ciThWallClock: "Wall-Clock",
    ciThAvgJob: "Avg Job",
    ciThConcurrency: "Concurrency",
    ciThEfficiency: "Efficiency",
  },
  zh: {
    title: "vllm-ascend CI Insight",
    searchPlaceholder: "搜索 PR 标题、编号...",
    allSeverities: "全部严重度",
    allCategories: "全部分类",
    severity: {
      critical: "严重",
      high: "高",
      medium: "中",
      low: "低",
      unknown: "未知",
    },
    category: {
      code: "业务代码问题",
      build: "工程脚本问题",
      infra: "基础设施问题",
      test: "测试用例问题",
      lint: "代码规范问题",
      compat: "兼容性问题",
      perf: "性能问题",
      other: "其他",
    },
    refresh: "刷新",
    loading: "加载中...",
    loadFailed: "加载报告失败。",
    noReports: "暂无报告。",
    failed: "个失败",
    reports: "份报告",
    totalFailedJobs: "失败 Job 数",
    avgConf: "平均置信度",
    severityBreakdown: "严重度分布",
    topWorkflows: "失败最多的 Workflow",
    categoryBreakdown: "问题分类统计",
    dailyTrend: "每日失败趋势",
    recentReports: "最近报告",
    pr: "PR",
    jobs: "任务",
    severityLabel: "严重度",
    categoryLabel: "分类",
    date: "日期",
    affectedRuns: "受影响 CI 运行",
    noRuns: "无运行记录。",
    analysis: "分析结果",
    failedJobs: "个失败任务",
    noAnalysis: "无分析结果。",
    errorSnippets: "关键错误片段：",
    rootCause: "根因分析",
    relatedFiles: "关联文件：",
    fixSuggestions: "修复建议：",
    noRootCause: "暂无根因分析。",
    detailFailed: "加载失败",
    author: "作者",
    analyzed: "CI 触发时间",
    lang: "English",
    confidence: "置信度",
    chartLoading: "图表加载中...",
    classified: "已分类",
    unclassified: "未分类",
    clickToDrill: "点击下钻",
    categoryDetailTitle: "分类详情",
    tabAnalysis: "问题分析",
    tabCIStats: "CI执行分析",
    ciAvgDuration: "平均执行时长",
    ciMedianDuration: "P50 时长",
    ciP90Duration: "P90 时长",
    ciP20Duration: "P20 时长",
    ciTotalRuns: "总运行数",
    ciSuccessRate: "成功率",
    ciQueueTime: "排队等待时长",
    ciQueueMeasured: "排队样本",
    ciNotCollected: "未采集",
    ciQueueNotCollectedHint: "排队时长需要 GitHub job created_at。旧版导出的 job 详情在重新抓取前可能没有该字段。",
    ciDurationDist: "Job 执行时长分布 (P50/P90/P20)",
    ciQueueWait: "Job 排队等待时长 (启动 - Job 创建)",
    ciSuccessByWF: "各 Workflow 成功率",
    ciSlowestJobs: "最慢 Job (Top 10)",
    ciDuration: "执行时长",
    ciQueueWaitLabel: "排队等待",
    ciSuccess: "成功",
    ciFailed: "失败",
    ciSkipped: "跳过",
    ciSeconds: "秒",
    ciMinutes: "分",
    ciTotalJobs: "Job 总数",
    ciJobP50: "Job P50",
    ciJobP90: "Job P90",
    ciJobP20: "Job P20",
    ciWfTotal: "Workflow 数",
    ciWfAvgWC: "Workflow 平均总时长",
    ciWfP50: "Workflow P50",
    ciWfP90: "Workflow P90",
    ciWfAvgJobs: "平均 Job/Workflow",
    ciWfEfficiency: "并行度",
    ciWfSuccess: "Workflow 成功率",
    ciTableTitle: "Workflow 运行明细",
    tabHealth: "健康概览",
    healthScoreLabel: "健康分",
    successRateLabel: "成功率",
    pipeline_pr: "PR CI",
    pipeline_nightly: "Nightly",
    pipeline_build: "构建/发布",
    pipeline_other: "其他",
    allPipelines: "全部分类",
    effort_low: "简单",
    effort_medium: "中等",
    effort_high: "困难",
    ciThWorkflow: "工作流",
    ciThStatus: "状态",
    ciThJobs: "Job数",
    ciThQueueTime: "最大排队时长",
    ciThWallClock: "总耗时",
    ciThAvgJob: "平均Job",
    ciThConcurrency: "最大并行",
    ciThEfficiency: "并行效率",
  },
};

let currentLang = localStorage.getItem("ascend-lang") || "en";

function t(key) {
  return I18N[currentLang]?.[key] || I18N.en[key] || key;
}

function tSeverity(sev) {
  return I18N[currentLang]?.severity?.[sev] || sev;
}

function tCategory(cat) {
  return I18N[currentLang]?.category?.[cat] || cat;
}

function toggleLang() {
  currentLang = currentLang === "zh" ? "en" : "zh";
  localStorage.setItem("ascend-lang", currentLang);
  document.getElementById("langToggle").textContent = t("lang");
  location.reload();
}
